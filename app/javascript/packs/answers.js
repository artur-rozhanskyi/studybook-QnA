$(document).on('turbolinks:load', function () {
  ...
  function file_block (attachment, index) {
    ...
    var input_hidden_label = $('<input>').attr({
    ...
    var input_checkbox_label = $('<input>').attr({
      name: 'answer[attachments_attributes][' + index + '][_destroy]',
      type: 'checkbox',
  ...
  function attachment_links (answer, block) {
    if (typeof answer.attachments !== 'undefined') {
      answer.attachments.forEach(function (attachment) {
        var link_to_file = $('<a>').attr('href', attachment.url).text(attachment.filename)
